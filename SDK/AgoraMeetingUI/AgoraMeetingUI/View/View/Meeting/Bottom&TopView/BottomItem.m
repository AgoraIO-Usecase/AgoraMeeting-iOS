...
- (instancetype)initWithCoder:(NSCoder *)coder
{
    ...
    if (self) {
        [[NSBundle meetingUIBundle] loadNibNamed:NSStringFromClass([self class]) owner:self options:nil];
...
- (void)configState:(BottomItemState)state timeCount:(NSInteger)timeCount {
    ...
    switch (state) {
        case BottomItemStateInactive:
            self.imageView.image = [UIImage meetingUIImageName:_info.inActiveImageName];
        ...
        case BottomItemStateActive:
            self.imageView.image = [UIImage meetingUIImageName:_info.activeImageName];
